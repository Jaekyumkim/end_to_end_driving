...
with graph.as_default():
    ...
    train_writer = tf.summary.FileWriter('HM_cropNvidiaCNN2D/train_summary', graph=graph)
    valid_writer = tf.summary.FileWriter('HM_cropNvidiaCNN2D/valid_summary', graph=graph)
...
checkpoint_dir = os.getcwd() + "/HM_cropNvidiaCNN2D"
...
with tf.Session(graph=graph, config=tf.ConfigProto(gpu_options=gpu_options)) as session:
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if valid_score < best_validation_score:
            saver.save(session, 'HM_cropNvidiaCNN2D/checkpoint-sdc-ch2')
            best_validation_score = valid_score
            print '\r', "SAVED at epoch %d" % epoch,
            with open("HM_cropNvidiaCNN2D/valid-predictions-epoch%d" % epoch, "w") as out:
            ...
            with open("HM_cropNvidiaCNN2D/test-predictions-epoch%d" % epoch, "w") as out:
