...
with graph.as_default():
    ...
    train_writer = tf.summary.FileWriter('arr/HM_cropNvidiaCNN2D_mobis+simulator/train_summary', graph=graph)
    valid_writer = tf.summary.FileWriter('arr/HM_cropNvidiaCNN2D_mobis+simulator/valid_summary', graph=graph)
...
checkpoint_dir = os.getcwd() + "/arr/HM_cropNvidiaCNN2D_mobis+simulator"
...
with tf.Session(graph=graph, config=tf.ConfigProto(gpu_options=gpu_options)) as session:
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if valid_score < best_validation_score:
            saver.save(session, 'arr/HM_cropNvidiaCNN2D_mobis+simulator/checkpoint-sdc-ch2')
            best_validation_score = valid_score
            print '\r', "SAVED at epoch %d" % epoch,
            with open("arr/HM_cropNvidiaCNN2D_mobis+simulator/valid-predictions-epoch%d" % epoch, "w") as out:
            ...
            with open("arr/HM_cropNvidiaCNN2D_mobis+simulator/test-predictions-epoch%d" % epoch, "w") as out:
        ...
        if epoch != NUM_EPOCHS - 1:
            ...
            for img, stats in train_predictions.items():
                result += stats[-1]
            print "Validation unnormalized RMSE:", np.sqrt(result / len(valid_predictions))
            with open("arr/HM_cropNvidiaCNN2D_mobis+simulator/train-predictions-epoch%d" % epoch, "w") as out:
