...
def readt_csv(filename, flag):
    ...
    with open(filename, 'r') as f:
        while True:
            ...
            ln = ln.strip().split(",")
            if flag is 0:
                lines.append([str(filename[:-10] + "../" + ln[0]), ln[1], 0, 0])
            else:      
                lines.append([str(filename[:-18] + "../" + ln[0]), ln[1], 0, 0])  

        return lines

def read_Udacitycsv(lines, flag):
    filename = ['./Challenge2/crop_steering_train.csv', './Challenge2/crop_steering_test.csv']
    select = ['train/', 'test/']
    f = open(filename[flag], 'r')
    while True:
        ln = f.readline()
        if not ln: break
        ln = ln.strip().split(",")
        pngname = os.path.abspath('./Challenge2/' + select[flag] + os.path.basename(ln[0]))
        lines.append([pngname, float(ln[1]), 0, 0])
    return lines
        
def read_Simulatorcsv(lines):
    filename = "/media/user/c45eb821-d419-451d-b171-3152a8436ba2/UdacitySimulator/car-behavioral-cloning/0_track/driving_log_png.csv"
    with open(filename, 'r')as f:
        f.readline()
        for i in range(24000):
            ...
            if not ln: break
            ln = ln.strip().split(",")
            lines.append([ln[0], str(float(ln[1])), 0, 0])
...
def process_csv(filename, val=5):
    ...
    lines = read_Udacitycsv(lines, 0)
    lines = read_Simulatorcsv(lines)
...
test_seq = readt_csv("../../../../data/drivePX/train/RMSE/TESTwithTRAIN",1) # interpolated.csv for testset filled with dummy values 
test_seq = map(lambda x: (x[0], np.float32(x[1:])), test_seq)
...
def apply_vision_simple(image, keep_prob, batch_size, seq_len, scope=None, reuse=None):
    ...
    with tf.variable_scope(scope, 'Vision', [image], reuse=reuse):
        net = slim.convolution(video, num_outputs=24, kernel_size=[1,5,5], stride=[1,2,2], padding="VALID")
        ...
        return net # aux[1-4] are residual connections (shortcuts) connections (shortcuts)
...
with graph.as_default():
    ...
    train_writer = tf.summary.FileWriter('arr/HM_cropNvidiaCNN2D_mobis+simulator+udacity/train_summary', graph=graph)
    valid_writer = tf.summary.FileWriter('arr/HM_cropNvidiaCNN2D_mobis+simulator+udacity/valid_summary', graph=graph)
...
checkpoint_dir = os.getcwd() + "/arr/HM_cropNvidiaCNN2D_mobis+simulator+udacity"
...
with tf.Session(graph=graph, config=tf.ConfigProto(gpu_options=gpu_options)) as session:
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if valid_score < best_validation_score:
            saver.save(session, 'arr/HM_cropNvidiaCNN2D_mobis+simulator+udacity/checkpoint-sdc-ch2')
            best_validation_score = valid_score
            print '\r', "SAVED at epoch %d" % epoch,
            with open("arr/HM_cropNvidiaCNN2D_mobis+simulator+udacity/valid-predictions-epoch%d" % epoch, "w") as out:
            ...
            with open("arr/HM_cropNvidiaCNN2D_mobis+simulator+udacity/test-predictions-epoch%d" % epoch, "w") as out:
        ...
        if epoch != NUM_EPOCHS - 1:
            print "Training"
            _, valid_predictions = do_epoch(session=session, sequences=train_seq, mode="train")
            result = np.float128(0.0)
            for img, stats in valid_predictions.items():
                result += stats[-1]
            print "Validation unnormalized RMSE:", np.sqrt(result / len(valid_predictions))
            with open("arr/HM_cropNvidiaCNN2D_mobis+simulator+udacity/train-predictions-epoch%d" % epoch, "w") as out:
                for img, pred in valid_predictions.items():
                    print >> out, "%s,%f" % (img, pred[1])
