...
#    input_images_norm = tf.cast(input_images, tf.float32) - tf.Variable([123.68, 116.779, 103.939])
    ...
    train_writer = tf.summary.FileWriter('chk/debug1JH/train_summary', graph=graph)
    valid_writer = tf.summary.FileWriter('chk/debug1JH/valid_summary', graph=graph)
...
checkpoint_dir = os.getcwd() + "chk/basic/5*1e-4"
...
#        best_validation_score = 100000000000000
        ...
        if valid_score < best_validation_score:
            saver.save(session, 'chk/basic/5*1e-4/checkpoint-sdc-ch2')
            best_validation_score = valid_score
            print '\r', "SAVED at epoch %d" % epoch,
            with open("chk/basic/5*1e-4/valid-predictions-epoch%d" % epoch, "w") as out:
            ...
            with open("chk/basic/5*1e-4/test-predictions-epoch%d" % epoch, "w") as out:
