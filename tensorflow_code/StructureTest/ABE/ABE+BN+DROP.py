...
#        pdb.set_trace()
        ...
        net = slim.convolution(net, num_outputs=512, kernel_size=[3,3], stride=[2,2], padding="SAME")
        net = slim.convolution(net, num_outputs=128, kernel_size=[1,1], stride=[1,1], padding="SAME")
        net = slim.convolution(net, num_outputs=256, kernel_size=[3,3], stride=[2,2], padding="SAME")
        net = slim.convolution(net, num_outputs=128, kernel_size=[1,1], stride=[1,1], padding="SAME")
        net = slim.convolution(net, num_outputs=256, kernel_size=[3,3], stride=[2,2], padding="SAME")
        # E + DROP
        net = slim.dropout(net,keep_prob)
        net = slim.fully_connected(tf.reshape(net, [batch_size * (LEFT_CONTEXT + seq_len), -1]), 256, activation_fn=tf.nn.relu)
...
#    input_images_norm = tf.cast(input_images, tf.float32) - tf.Variable([123.68, 116.779, 103.939])
    ...
    train_writer = tf.summary.FileWriter('chk/basic/ABE+BN+DROP/train_summary', graph=graph)
    valid_writer = tf.summary.FileWriter('chk/basic/ABE+BN+DROP/valid_summary', graph=graph)
...
checkpoint_dir = os.getcwd() + "/chk/basic/ABE+BN+DROP"
...
#        best_validation_score = 100000000000000
        ...
        if valid_score < best_validation_score:
            saver.save(session, 'chk/basic/ABE+BN+DROP/checkpoint-sdc-ch2')
            best_validation_score = valid_score
            print '\r', "SAVED at epoch %d" % epoch,
            with open("chk/basic/ABE+BN+DROP/valid-predictions-epoch%d" % epoch, "w") as out:
            ...
            with open("chk/basic/ABE+BN+DROP/test-predictions-epoch%d" % epoch, "w") as out:
